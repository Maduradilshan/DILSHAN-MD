...
smd({
        ...
        async(DILSHAN.bot, msg, text , {isCreator}) => {
            if (!msg.isGroup) return msg.reply(tlang().group);
          let check = text ? text : '';
...
smd({
 ...
 async(DILSHAN, msg, text,{ isCreator }) => {
      ...
      let metadata = await DILSHAN.bot.groupMetadata(msg.chat);
      var ppuser;
      let num = msg.sender;
      ...
      try {  ppuser = await DILSHAN.bot.profilePictureUrl(num, 'image') }catch { ppuser = 'https://telegra.ph/file/93f1e7e8a1d7c4486df9e.jpg' ; }
      return await DILSHAN.bot.sendMessage(msg.chat, { image: { url: ppuser }, caption: welcome_messages,mentions: [num] } )
 ...
 async(DILSHAN, msg, text,{ isCreator }) => {
    ...
    try {  ppuser = await DILSHAN .bot.profilePictureUrl(num, 'image') }catch { ppuser = 'https://telegra.ph/file/93f1e7e8a1d7c4486df9e.jpg' ; }
        ...
        return await DILSHAN.bot.sendMessage(msg.chat, { image: { url: ppuser }, caption: goodbye_messages, mentions: [num] })
...
if(ty){  
    ...
    async(DILSHAN, msg, text) => {
        var bio = await DILSHAN.bot.fetchStatus(msg.sender) || {};
        var bioo = bio.status ;
        let meh = msg.sender;
...
let pfp;
        try {pfp = await DILSHAN.bot.profilePictureUrl(msg.sender, "image");} 
...
`;
        ...
        return await DILSHAN.bot.sendMessage(msg.chat, buttonMessage, { quoted: msg });
...
userq.level + 1
        ...
        try { ppuser = await DILSHAN.bot.profilePictureUrl(msg.sender, "image");}
...
smd({ on: "text" }, async(DILSHAN, msg) => {
    ...
    if (hasLeveledUp){
        ...
        {
            await DILSHAN.bot.sendMessage(msg.chat, { image: {  url: await botpic() },
        caption: `
╔════◇
║ *Wow,Someone just*
║ *leveled Up huh⭐*
