...
smd({
		...
		for ( count ; count < limit ; count++) 
		{
		 // if (count >= limit) break;
		  ...
		  if(isSticker) { let a = await getBuffer(sticUrl); await citel.reply(a, { packname: Config.packname, author: "DILSHAN-Md"  }, "sticker");} 
...
smd({pattern: "gitclone",desc: "Downloads apks  .",category: "downloader",filename: __filename,use: '<git url>', },
        async(DILSHAN, citel, text) => {
	if (!text) return await citel.send('*Provide Repo Url, Ex:- _.gitclone https://github.com/Maduradilshan/DILSHAN-MD_*') 
...
smd({pattern: "tts",desc: "text to speech.",category: "downloader",filename: __filename,use: '<Hii,this is DILSHAN>',},
        async(DILSHAN, citel, text) => {
            ...
            return DILSHAN.bot.sendMessage(citel.chat,{audio: {url: ttsurl}, mimetype: "audio/mpeg", fileName: `tts DILSHAN Md.m4a` },{quoted: citel } );
        }
...
async(DILSHAN, citel, text) => {
  ...
  if (!urlYt) { let yts = require("DILSHAN-pack"),search = await yts(text),anu = search.videos[0];urlYt = anu.url;  }
...
smd({pattern: "pint",desc: "Downloads image from pinterest.",category: "downloader",filename: __filename,use: '<text|image name>',},
        async(DILSHAN, citel, text) => {
            if (!text) return citel.send(`What picture are you looking for?`) && DILSHAN.bot.sendMessage(citel.chat, { react: {  text: '❌', key: citel.key  }  })
            try {
                let anu = await pinterest(text)
                let result = anu[Math.floor(Math.random() * anu.length)]
                let buttonMessage = {image: { url: result },caption: Config.caption ,
                    contextInfo: { externalAdReply: { title: `Here it is✨`,body: `${Config.ownername}`,thumbnail: log0,mediaType: 2, mediaUrl: ``,sourceUrl: `youtube.com/c/DILSHANTechInfo`},}
                }
                return DILSHAN.bot.sendMessage(citel.chat, buttonMessage, {  quoted: citel })
           ...
           /*
            ...
            for (let i of search.all) {
                listSerch.push({
                    ...
                    description: `*DILSHAN-MD* / ${i.timestamp}`
                })
            }
            const sections = [
            ...
            const listMessage = {
                ...
                title: ``,
                buttonText: "Songs",
                mentions: await DILSHAN.bot.parseMention(teskd),
                sections
            }
...
async(DILSHAN, citel, text) => {
      ...
      if (!text || !vid[0]) return await citel.reply(`❌Please provide me a url`);
        try {
          ...
          try{    
            let file = await yt.download(vid[1],{type : "audio",quality:"best"})
            file ? await DILSHAN.bot.sendMessage(citel.chat, {audio: {url : file } ,mimetype: 'audio/mpeg', }) :  await citel.send("*_Video not Found_*"); 
            try{fs.unlinkSync(file)}catch{}
          }catch(err){
...
async(DILSHAN, citel, text) => {
  ...
  }catch(err){ return await DILSHAN.bot.sendMessage(citel.chat, {document: {url : file } ,mimetype: 'audio/mpeg',fileName: `DILSHAN-Md ${vid[1]}.mp3`,caption: `${Config.caption}` }) ;try{ return await fs.unlinkSync(file); } catch{}       } 
})
